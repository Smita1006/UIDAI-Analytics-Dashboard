...
export function TimeSeriesAnalysis({ data }: TimeSeriesAnalysisProps) {
  ...
  const processChartData = () => {
    if (!timeSeriesData?.daily_trends) return [];

    return timeSeriesData.daily_trends.map((day: any) => ({
      ...
      // Calculate ratios
      biometric_ratio: day.biometric_count
        ? (day.biometric_count / day.total_count) * 100
        : 0,
      demographic_ratio: day.demographic_count
  ...
  const processWeeklyData = () => {
    if (!weeklyData?.weekly_patterns) return [];
    ...
    return weeklyData.weekly_patterns.map((pattern: any, index: number) => ({
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Daily Trends Chart */}
      {selectedView === "daily" && chartData.length > 0 && (
        <Card>
          <CardHeader>
      ...
      {selectedView === "weekly" && weeklyChartData.length > 0 && (
        <Card>
          <CardHeader>
